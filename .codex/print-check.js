...
// src/app/dashboard/hr/utils.ts
var getAttendanceDays = (record) => Math.max(record.workingDays - record.weekOffDays - record.leaveDays, 0);
var formatCurrency = (value) => new Intl.NumberFormat("en-IN", {
  style: "currency",
  currency: "INR",
...
var formatMonthLabel = (month) => {
  if (!/^\d{4}-\d{2}$/.test(month)) return month;
  const parsed = /* @__PURE__ */ new Date(`${month}-01T00:00:00`);
...
// src/app/dashboard/hr/print.ts
var openSalarySlipPrintWindow = (employee, record) => {
  ...
  const paymentModeLabel = record.paymentMode === "full_payment" ? "Full Payment" : "Prorated";
  const generatedAt = format(/* @__PURE__ */ new Date(), "dd MMM yyyy, hh:mm a");
  const html = `
    <html>
      <head>
        <meta charset="utf-8" />
          ...
          <div class="sheet">
            ...
            <div class="content">
                ...
                <div>
                  <div class="table-card">
                    ...
                    <table>
                      <thead><tr><th>Component</th><th>Amount</th></tr></thead>
                      <tbody>
                        <tr><td>PF</td><td>${formatCurrency(record.deductions.pf)}</td></tr>
                        <tr><td>ESI</td><td>${formatCurrency(record.deductions.esi)}</td></tr>
                        <tr><td>Professional Tax</td><td>${formatCurrency(record.deductions.professionalTax)}</td></tr>
                        <tr><td>TDS</td><td>${formatCurrency(record.deductions.tds)}</td></tr>
                        <tr><td>Other Deductions</td><td>${formatCurrency(record.deductions.other)}</td></tr>
                      </tbody>
                    </table>
                  </div>
                  <div class="summary-card">
                    ...
                    <div class="body">
                      ...
                      <p><strong>Week Off:</strong> ${record.weekOffDays} | <strong>Leave:</strong> ${record.leaveDays} | <strong>LOP:</strong> ${record.lopDays}</p>
              ...
              <div class="bank-grid">
                ...
                <div class="bank-card">
                  <div class="label">UAN / ESI</div>
                  <div class="value">${employee.uanNumber || "-"} / ${employee.esiNumber || "-"}</div>
                </div>
              </div>

              ${record.notes ? `<div class="note-card"><div class="label">Payroll Notes</div><p>${record.notes}</p></div>` : ""}
            </div>
          </div>
        </div>
        <script>
