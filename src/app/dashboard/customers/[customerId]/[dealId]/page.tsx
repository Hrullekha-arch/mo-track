...
import { getDealById, updateDealProducts, getQuotationsForDeal, getOrdersForDeal, addVisitAction, getVisitsForDeal, addMeasurementAction, getMeasurementsForDeal, addCpdAction, getCpdsForDeal, createDealOrderAction, createSelectionAction, getSelectionsForDeal, getProductsByIds } from "./actions";
...
function ProductForm({ initialProducts, customerId, dealId, onRefresh, deal, customer, cpds, quotations, orders }: { initialProducts: DealProduct[], customerId: string, dealId: string, onRefresh: () => void, deal: Deal, customer: Customer, cpds: Cpd[], quotations: Quotation[], orders: DealOrder[] }) {
    ...
    const handlePreviewSelection = async (selection: Selection) => {
        setAllProducts(await getProductsByIds(selection.productIds));
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {selections.map(selection => {
                                    const totalQty = selection.products.reduce((sum, p) => sum + (Number(p.quantity) || 0), 0);
                                    const totalAmount = selection.products.reduce((sum, p) => sum + ((Number(p.quantity) || 0) * (Number(p.mrp) || 0)), 0);
                                    return (
                                        <TableRow key={selection.id}>
                                            <TableCell className="font-medium">#{selection.id}</TableCell>
                                            <TableCell>{format(new Date(selection.createdAt), "PPP")}</TableCell>
                                            <TableCell className="text-right">{totalQty.toFixed(2)}</TableCell>
                                            <TableCell className="text-right">₹{totalAmount.toFixed(2)}</TableCell>
                                            <TableCell className="text-right">
                                                <Button variant="ghost" size="icon" onClick={() => handlePreviewSelection(selection)}>
        ...
        />
         <Dialog open={!!selectedSelection} onOpenChange={() => setSelectedSelection(null)}>
            <DialogContent className="max-w-4xl">
                <DialogHeader>
                    <DialogTitle>Selection Preview #{selectedSelection?.id}</DialogTitle>
                </DialogHeader>
                <div id="printable-selection-content" className="py-4">
                    {selectedSelection && (
                        <PrintableSelection 
                ...
                <DialogFooter>
                    <Button variant="ghost" onClick={() => setSelectedSelection(null)}>Close</Button>
