...
import { formatCurrency, hasSalaryConfig, updatedTimeLabel } from "../utils";
    ...
    <Card className="border-slate-200">
      <CardHeader className="gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <CardDescription>
            Generate payroll, update attendance adjustments, and create printable salary slips.
          </CardDescription>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            title={!hasAttendance ? "Upload attendance first to generate payroll" : undefined}
          >
            {bulkGenerating ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileText className="h-4 w-4" />}
            Generate Pending Payroll
          </Button>
          <Button
            type="button"
            variant="outline"
        ...
        {payrollLoading ? (
          ...
          <div className="flex flex-col items-center gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-6 py-10 text-center">
            ...
            <p className="max-w-sm text-xs text-amber-600">
              Payroll can only be generated for months where attendance data has been imported.
              Please upload biometric or manual attendance for this month first.
            </p>
          </div>
