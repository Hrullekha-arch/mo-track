...
export function usePayrollHandlers({
  ...
  const openPayrollDialog = (employee: HrEmployee) => {
    ...
    if (!existing && attSummary.totalDays === 0) {
      toast({
        variant: "destructive",
        title: "Attendance not imported",
        description: "Payroll can be calculated only for active employees whose biometric code attendance is imported for this month.",
      });
  ...
  const generatePendingPayroll = async () => {
    ...
    if (!pendingRows.length) {
      toast({
        ...
        description: "Payroll is already generated for all active employees with imported attendance this month.",
      });
      return;
    }
