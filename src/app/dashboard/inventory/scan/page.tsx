...
function AllocationDialog({ 
    ...
    const onSubmit = async (data: AllocationFormValues) => {
        ...
        // This is a complex operation. We'll simplify for now.
        // We will just mark the selected stock lengths as "used" by setting their quantity to 0.
        // This is a simplified version of the full allocation logic.
        try {
            const batchPromises = data.selectedLengths.map(sl => {
                const transaction: Omit<StockTransaction, 'id'> = {
                    ...
                    bcn: stockItem.bcn || '',
                    type: 'deduction',
                    quantityChange: -sl.length, // Deduct the full length
                    orderId: data.orderId || 'Manual Allocation',
                    lengths: [sl.length],
                    createdAt: new Date().toISOString(),
                    createdBy: user.name,
                    status: 'cut', // Mark as used immediately
                };
